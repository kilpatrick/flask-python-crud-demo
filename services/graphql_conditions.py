...
def create_condition(school_id: str, request_body: dict) -> [dict, dict]:
  import uuid
  ...
  db_seed["conditions"][new_id] = new_condition
  return new_condition, TODO_METADATA

def read_all_conditions(school_id: str) -> [dict, dict]:
  conditions_filtered_by_school = []
  for condition_id in db_seed["conditions"]:
    record = db_seed["conditions"][condition_id]
    ...
    if school_id  and record.get("deleted_at") == None and row_school_id and row_school_id == school_id:
      conditions_filtered_by_school.append(record)
  return conditions_filtered_by_school, TODO_METADATA

def read_condition_by_id(school_id: str, condition_id: str) -> [dict, dict]:
  record = db_seed["conditions"].get(condition_id)
...
def update_condition_by_id(school_id: str, condition_id: str, update_body: dict) -> [dict, dict]:
  record = db_seed["conditions"].get(condition_id)
  if record and record.get("deleted_at") == None and record.get("school_id") == school_id:
    ...
    updated_record = {**db_seed["conditions"][condition_id], **update_body}
    updated_record["updated_at"] = datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')
    db_seed["conditions"][condition_id] = updated_record
    return db_seed["conditions"][condition_id], TODO_METADATA
  return None, TODO_METADATA

def delete_condition_by_id(school_id: str, condition_id: str) -> [dict, dict]:
  record = db_seed["conditions"].get(condition_id)
  if record and record.get("deleted_at") == None and record.get("school_id") == school_id:
    mocked_cur_date_getter_setter = datetime.now(timezone.utc).isoformat().replace('+00:00', 'Z')
    updated_record = {**db_seed["conditions"][condition_id], "deleted_at": mocked_cur_date_getter_setter}
    db_seed["conditions"][condition_id] = updated_record
